...
// so your module needs to plug into the system and only take care of orders and ingredients. And since this is the future and food is printed 
// with nano-particle beams, all ingredients are microelements - protein, carbohydrate, fat, and flavours. The library of recipes includes the following meals:
...
// so the internal state must be preserved throughout the entire session.
...
function solution() {
    ...
    return (input) => {
        const [command, item, qty] = input.split(' ');
        ...
        if (command === 'restock') {
            ...
            Object.keys(recipe).forEach(ingredient => stock[ingredient] -= recipe[ingredient] * quantity);
            return 'Success';
